...
class PacketGenerator(Module):
    def __init__(self, dw=32):
        self.source = stream.Endpoint(eth_udp_user_description(dw))

        self.sync += [
            self.source.length.eq(dw//8),
            self.source.ip_address.eq(convert_ip("192.168.1.100")),
            self.source.src_port.eq(5000),
            self.source.dst_port.eq(5000),
            If(self.source.ready,
                self.source.last.eq(1),
                self.source.valid.eq(1),
                self.source.data.eq(0xdeadbeef)
            )
        ]
...
class BaseSoC(SoCCore):
    def __init__(self, device="85F", sys_clk_freq=int(75e6),
        ...
        **kwargs):
        ...
        bpu = BPU(data_width=self.bus.data_width, adr_width=self.bus.address_width)
        ...
        platform.add_extension([
            ("hyperram", 0,
                Subsignal("dq", Pins("C16 B17 C18 B19 C19 A19 A18 A17"), IOStandard("LVCMOS33")),
                Subsignal("rwds", Pins("A16"), IOStandard("LVCMOS33")),
                Subsignal("cs_n", Pins("C14"), IOStandard("LVCMOS33")),
                Subsignal("rst_n", Pins("A15"), IOStandard("LVCMOS33")),
                Subsignal("clk", Pins("E14"), IOStandard("LVCMOS33")),
                Misc("SLEWRATE=FAST"),
            )
        ])
        ...
        platform.add_extension([
            ("mipi-csi", 0,
                Subsignal("clk", Pins("C25"), IOStandard("LVDS25")),
                Subsignal("d0", Pins("D25"), IOStandard("LVDS25")),
                Subsignal("d1", Pins("F25"), IOStandard("LVDS25")),
                Misc("SLEWRATE=FAST"),
            )
        ])
        ...
        platform.add_extension([
            ("i2c", 0,
                Subsignal("scl", Pins("U25"), IOStandard("LVCMOS33")),
                Subsignal("sda", Pins("T25"), IOStandard("LVCMOS33")),
            )
        ])
        ...
        if with_ethernet or with_etherbone:
            ...
            if with_etherbone:
                ...
                dw = 32
                #udp_port = self.ethcore_etherbone.udp.crossbar.get_port(5000, dw=dw, cd="eth_tx")
                udp_port = self.ethcore_etherbone.udp.crossbar.get_port(5000, dw=dw)

                generator = PacketGenerator(dw=dw)
                #generator = ClockDomainsRenamer("eth_tx")(generator)
                self.submodules.generator = generator

                self.comb += generator.source.connect(udp_port.sink)

                """
                self.sync += [
                    udp_port.sink.length.eq(dw//8),
                    udp_port.sink.ip_address.eq(convert_ip("192.168.1.100")),
                    udp_port.sink.src_port.eq(5000),
                    udp_port.sink.dst_port.eq(5000),
                    If(udp_port.sink.ready,
                        udp_port.sink.last.eq(1),
                        udp_port.sink.valid.eq(1),
                        udp_port.sink.data.eq(0xdeadbeef)
                    )
                ]
                """

                """
                udp_stream = LiteEthUDPStreamer(
                ...
                udp_stream = ClockDomainsRenamer("eth_tx")(udp_stream)
                self.submodules.udp_stream = udp_stream

                stream = StreamTest(dw=32)
                stream = ClockDomainsRenamer("eth_tx")(stream)
                self.submodules.stream = stream
                
                stream.source.connect(udp_stream.sink)
                """

                #udp_stream.source.connect(udp_stream.sink)

        i2c_pads = platform.request("i2c")
        self.submodules.stemma = I2CMasterA(i2c_pads, sys_clk_freq, 3.4e6)
